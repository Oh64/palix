...
package fr.ohsq.palix.particle;
...
public class PalixyparticulareParticle {
	public static final BasicParticleType particle = new BasicParticleType(false);

	@SubscribeEvent
	public static void registerParticleType(RegistryEvent.Register<ParticleType<?>> event) {
		event.getRegistry().register(particle.setRegistryName("palixy_particule"));
	}
	...
	private static class CustomParticle extends SpriteTexturedParticle {
		...
		public void tick() {
			super.tick();
			if (!this.isExpired) {
				this.setSprite(this.spriteSet.get((this.age / 154) % 1 + 1, 1));
			}
		}
	}
